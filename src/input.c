...
void deal_with_flags (int argc, char** argv, char** input, SortMethod* mode, int* size, bool* output_to_file, bool* kill_on_null) {
    bool error = false;
    bool help = false;
    ...
    if (error || help) {
        ...
        if (help) {
            printf( "Quando sem <file>, ou quando <file> é -, lê da entrada padrão.\n"
                    ...
                    "    -k, --kill                   Aborta o programa caso algum sort falhe em alocar memória\n"
                    );
        }
        exit(0);
    }
